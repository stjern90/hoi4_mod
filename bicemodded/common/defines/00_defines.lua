...
NGame = {
	...
	GAME_SPEED_SECONDS = { 1, 0.5, 0.25, 0.1, 0.0 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
...
NDiplomacy = {
	...
	WARGOAL_WORLD_TENSION_REDUCTION = -0.5,			-- Reduction of pp cost for wargoal at 100% world tension, scales linearly # was -0.5
	...
	MIN_WARGOAL_JUSTIFY_COST = 2.5,					-- It always takes atleast 10 days to justify a wargoal
	...
	TENSION_CAPITULATE = 0.7,						-- Scale of the amount of tension created by a countries capitulation.
	...
	OPINION_FOR_DEMO_FROM_WT_GENERATION = -1.2,		-- How much less do democracies like us if we generate world tension
...
NCountry = {
	...
	CONVOY_RANGE_FACTOR = 0.3,                        -- How much range affects convoy need for resource trades and supply
	...
	CIVIL_WAR_INVOLVEMENT_MIN_TENSION = 0.7,		-- base value of world tension to involve other sides to the civil war
	UNCAPITULATE_LEVEL = 0.2,                       -- if we reclaim this much and our capital we reset capitulate status
	BASE_SURRENDER_LIMIT = 0.65,						-- Base level of occupation required for country surrender
	...
	MAX_HEROES_BEING_KILLED_WAR_SUPPORT_IMPACT = -0.25,				-- Max total penalty from war heroes manpower lost
	WAR_SUPPORT_FROM_CASUALTIES = 0.02,							-- Base value (inverted) for calculating heroes being killed
	...
	MAX_CONVOYS_BEING_RAIDED_WEEKLY_WAR_SUPPORT_PENALTY = -0.005,	-- Max penalty that will gained per week from trade convoy raided
	CONVOYS_BEING_RAIDED_WEEKLY_WAR_SUPPORT_PENALTY_DECAY = 0.0001,	-- Weekly decay of trade convoy raided war support penalty
...
NResistance = {
	...
	INITIAL_STATE_COMPLIANCE = 5.0,							-- initial compliance percentage of a state once it is captured
	...
	RESISTANCE_DECAY_BASE = 0.07, -- base resistance decay
	...
	RESISTANCE_DECAY_MODIFIER_FACTORS = {  -- resistance decay modifier when resistance hits a certain percentage
		10, -80, -- below 10% it has a -80% modifier on decay
		20, -45, -- below 20% it has a -45% modifier on decay
	},
	...
	RESISTANCE_GROWTH_BASE = 0.22, -- base resistance grow
	...
	COMPLIANCE_GROWTH_IS_AT_PEACE = 8, -- compliance growth buff at peace
	...
	RESISTANCE_RATIO_DIFF_TO_SPREAD = 0.33, -- resistance diff between two neighbour states will spread by this ratio ( from highest resistance states to lower ones and it will only spread once to a state)
	...
	GARRISON_STR_POW_MANPOWER = 2.5,	--Scales impact of manpower deficiency by raising that deficiency to the number here. Formula: efficiency = 1.0 - manpower_deficiency^GARRISON_STR_POW_MANPOWER
	...
	SUPPRESSION_NEEDED_BY_RESISTANCE_POINT = 0.70, -- Number of suppression point we need for each 1% of resistance
	...
	GARRISON_MANPOWER_LOST_BY_ATTACK = 0.013, 	-- Ratio of manpower lost by garrison at each attack on garrison (this number will be reduced by the hardness of garrison template)
	GARRISON_EQUIPMENT_LOST_BY_ATTACK = 0.027, 	-- Ratio of equipment lost by garrison at each attack on garrison (this number will be reduced by the hardness of garrison template)
	...
	MANPOWER_BUFFER_TO_NOT_GIVE_MINOR = 0.4, -- To determine how much AI can give as foreign manpower, we calculate how much manpower we use, and add this buffer. The result is what we want to keep, for minor countries. So higher this number is, lower we will give Manpower.
...
NProduction = {
	...
	BASE_FACTORY_SPEED_MIL = 3.0, 				-- Base factory speed multiplier (how much hoi3 style IC each factory gives).
	...
	MIN_POSSIBLE_TRAINING_MANPOWER = 80000,	-- How many deployment lines minimum can be training
	...
	SHIP_REFIT_MAX_PROGRESS_TO_CANCEL = 0.6,			-- Maximum refitting progress % that we still allow to cancel wihtout having to scuttle the ship.
...
NTechnology = {
	...
	MAX_TECH_SHARING_BONUS = 0.4, 			-- Max technology sharing bonus that can be applied instantly
...
NPolitics = {
	...
	ARMY_LEADER_COST = 6,					-- command power cost for recruiting new leaders, 'this value' * number_of_existing_leaders_of_type
	NAVY_LEADER_COST = 6,					-- command power cost for recruiting new leaders, 'this value' * number_of_existing_leaders_of_type
...
NBuildings = {
	...
	NAVALBASE_REPAIR_MULT = 0.12,		-- Each level of navalbase building repairs X strength. The value is spread on all ships needed reparation. Fe Navalbase lvl 5 x 0.5 str repair = 2.5 str repaired over 10 ships, so each ship repair hourly 0.25 str.
	...
	INFRASTRUCTURE_RESOURCE_BONUS = 0.04, -- multiplicative resource bonus for each level of (non damaged) infrastructure
	SUPPLY_ROUTE_RESOURCE_BONUS = 0.04,   -- multiplicative resource bonus for having a railway/naval connection to the capital
...
NMilitary = {
	...
	DIVISIONAL_COMMANDER_TRAIT_XP_REQUIREMENT = 500.0,	--Get a trait if any valid options & xp gained >= this
	...
	ZERO_ORG_MOVEMENT_MODIFIER = -0.3,		-- speed impact at 0 org.
	...
	MIN_DIVISION_BRIGADE_HEIGHT = 1,		-- Min height of regiments in division designer.
	...
	BASE_DIVISION_BRIGADE_GROUP_COST = 2, 	--Base cost to unlock a regiment slot,
	...
	LAND_COMBAT_STR_ARMOR_DEFLECTION_FACTOR = 0.67, -- damage reduction if armor outclassing enemy
	LAND_COMBAT_ORG_ARMOR_DEFLECTION_FACTOR = 0.67, -- damage reduction if armor outclassing enemy
	LAND_COMBAT_COLLATERAL_FORT_FACTOR = 0.0012,		   -- Factor to scale collateral damage to infra and forts with.
	LAND_COMBAT_COLLATERAL_INFRA_FACTOR = 0.001,	-- Factor to scale collateral damage to infra with.
	...
	ATTRITION_WHILE_MOVING_FACTOR = 0.08,
	...
	ENEMY_AIR_SUPERIORITY_DEFENSE = 0.64,	       -- more AA attack will approach this amount of help (diminishing returns)
	ENEMY_AIR_SUPERIORITY_DEFENSE_STEEPNESS = 60, -- how quickly defense approaches the max impact diminishing returns curve
	...
	TRAINING_EXPERIENCE_SCALE = 75.0,				-- was 62.0 testing a bit more cause training needs so much more equipment than vanilla game
	TRAINING_ORG = 0.2,
	ARMY_EXP_BASE_LEVEL = 1,
	UNIT_EXP_LEVELS = { 0.1, 0.3, 0.75, 0.9 },		-- Experience needed to progress to the next level Changed vets to 0.55 instead 0.7 ***
	...
	PARACHUTE_FAILED_EQUIPMENT_DIV = 10.0,		   -- When the transport plane was shot down, we drop unit with almost NONE equipment
	PARACHUTE_FAILED_MANPOWER_DIV = 10.0,		   -- When the transport plane was shot down, we drop unit with almost NONE manpower
	PARACHUTE_FAILED_STR_DIV = 5.0,			   -- When the transport plane was shot down, we drop unit with almost NONE strenght
	...
	SUPPLY_ORG_MAX_CAP = 0.33,                     -- Max organization is factored by this if completely out of supply
	...
	NUKE_MIN_DAMAGE_PERCENT = 0.4,					-- Minimum damage from nukes as a percentage of current strength/organisation
	...
	COMBAT_SUPPLY_LACK_ATTACKER_ATTACK = -0.30,     -- attack combat penalty for attacker if out of supply
	...
	PROMOTE_LEADER_CP_COST = 20.0,					-- cost of promoting a leader
	...
	ARMY_NAVAL_TRANSFER_FUEL_MULT = 0.66,			-- fuel consumption ratio while naval transferring
	ARMY_STRATEGIC_DEPLOYMENT_FUEL_MULT = 0.66,		-- fuel consumption ratio while doing strategic deployment
	ARMY_IDLE_FUEL_MULT = 0.33,						-- fuel consumption ratio while just existing
	FUEL_EFFICIENCY_RAID_MULTIPLIER = 1.1,			-- convoy raid multiplier for fuel sunk
...
NAir = {
	...
	AIR_WING_BOMB_DAMAGE_FACTOR = 1.6,					-- Used to balance the damage done while bombing.	
	BIGGEST_AGILITY_FACTOR_DIFF = 3.5,					-- biggest factor difference in agility for doing damage (caps to this)
	BIGGEST_SPEED_FACTOR_DIFF = 2.75,					-- biggest factor difference in speed for doing damage (caps to this)
	TOP_SPEED_DAMAGE_BONUS_FACTOR = 0.025,				-- A factor for scaling the top speed of a plane into damage buff
	...
	COMBAT_BETTER_SPEED_DAMAGE_INCREASE = 0.50, 		-- How much the better Speed (than opponent's) can reduce increase our damage to them.
	...
	COMBAT_MAX_WINGS_AT_ONCE_PORT_STRIKE = 300,        -- we can really pounce a naval strike and escalate
	...
	COMBAT_DAMAGE_SCALE = 0.4,							-- Higher value = more shot down planes
	...
	NAVAL_STRIKE_DAMAGE_TO_ORG = 2.22,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Organisation reduction.
	...
	ACCIDENT_CHANCE_BALANCE_MULT = 0.20,					-- Multiplier for balancing how often the air accident really happens. The higher mult, the more often.
	...
	BASE_STRATEGIC_BOMBING_HIT_SHIP_CHANCE = 0.02,		-- Chance to hit a ship in port when it is bombed.
	BASE_STRATEGIC_BOMBING_HIT_SHIP_DAMAGE_FACTOR = 250,
	...
	SUPPLY_NEED_FACTOR = 0.4, 							-- multiplies supply usage
	...
	NAVAL_STRIKE_BASE_STR_TO_PLANES_RATIO = 0.06,		-- Max airbombers to do port strike comparing to strength
	...
	AIR_WING_XP_AIR_VS_AIR_COMBAT_GAIN = 0.85, 							--Wings in combat gain extra XP	
	...
	AIR_WING_COUNTRY_XP_FROM_TRAINING_FACTOR = 0.006, 					--Factor on country Air XP gained from wing training
	AIR_WING_XP_TRAINING_MISSION_ACCIDENT_FACTOR = 1.3, 				--Training exercises cause more accidents
	...
	DISRUPTION_FACTOR_CARRIER = 2.0,							-- multiplier on disruption damage to scale its effects on carrier vs carrier planes
	...
	NAVAL_MINES_SWEEPING_SPEED_MULT = 0.018,						-- Value used to overall balance of the speed of sweeping naval mines
...
NNavy = {
	...
	MAX_SUBMARINES_PER_AUTO_TASK_FORCE = 8,						-- maximum number of submarines the auto-task force creation will put together when designing wolfpack
	...
	COMBAT_TORPEDO_CRITICAL_DAMAGE_MULT = 3.5,						-- multiplier to damage when got critical hit from torpedo. (Critical hits are devastating as usualy torpedo_attack are pretty high base values).
	COMBAT_DAMAGE_TO_STR_FACTOR = 0.43,								-- casting damage value to ship strength multiplier. Use it ot balance the game difficulty.
	...
	ANTI_AIR_TARGETTING_TO_CHANCE = 0.21,							-- Balancing value to convert averaged equipment stats (anti_air_targetting and naval_strike_agility) to probability chances of airplane being hit by navies AA.
	...
	AMPHIBIOUS_INVADE_SPEED_BASE = 0.25, 							-- every hour movement progress on amphibious invasion
	AMPHIBIOUS_INVADE_MOVEMENT_COST = 24.0, 						-- total progress cost of movement while amphibious invading
	...
	NAVAL_SPEED_MODIFIER = 0.16,	                    				-- basic speed control
	...
	NAVAL_TRANSFER_BASE_NAVAL_DIST_ADD = 80,						-- Extra cost for naval movement ( compared to land movement ) when deciding what ports to use for a naval transfer
	NAVAL_TRANSFER_BASE_NAVAL_DIST_MULT = 15,						-- Multiplier for the cost of naval movement ( compared to land movement ) when deciding what ports to use for naval transfer
	...
	ENEMY_AIR_SUPERIORITY_IMPACT = -0.25,           					-- effect on ship efficiency due to enemy air superiorty
	...
	MINIMUM_SHIP_SPEED = 2.0,										-- slowest speed a ship can have
	...
	CONVOY_SPOTTING_COOLDOWN_MIN = 64, -- minimum cooldown time
	CONVOY_SPOTTING_COOLDOWN_MAX = 168, -- maximum cooldown time
	CONVOY_SPOTTING_COOLDOWN_MIN_FROM_EFFICIENCY = 24, -- clamped min value after screening efficiency has been applied
	...
	SUPREMACY_PER_SHIP_PER_MANPOWER = 0.06,							-- supremacy of a ship is calculated using its IC, manpower and a base define
	...
	NAVAL_MINES_PLANTING_SPEED_MULT = 0.008,						-- Value used to overall balance of the speed of planting naval mines
	NAVAL_MINES_SWEEPING_SPEED_MULT = 0.01,						-- Value used to overall balance of the speed of sweeping naval mines
	NAVAL_MINES_DECAY_AT_PEACE_TIME = 5.0,							-- How fast mines are decaying in peace time. Planting mines in peace time may be exploitable, so it's blocked atm. That's why after war we should decay them too.
	...
	NAVAL_MINES_INTEL_DIFF_FACTOR = 0.35,					-- Better our decryption over enemy encryption will reduce the penalties from the enemy mines in the region. This value is a factor to be used for balancing.
	NAVAL_MINES_NAVAL_SUPREMACY_FACTOR = 0.5,						-- Factor for max amount of mines increasing naval supremacy
	...
	NAVAL_MINES_ACCIDENT_CRITICAL_HIT_DAMAGE_SCALE = 2.5,			-- Scale the value below in case of critical hit (caused by naval mines)
	NAVAL_MINES_ACCIDENT_STRENGTH_LOSS = 75.0,						-- Amount of strength loss when hit by naval mine
	NAVAL_MINES_ACCIDENT_ORG_LOSS_FACTOR = 0.3,						-- Amount of strength loss when hit by naval mine
	...
	TRAINING_ACCIDENT_CRITICAL_HIT_DAMAGE_SCALE = 2.5,				-- Scale the value below in case of critical hit
	...
	TRAINING_MIN_STRENGTH = 0.25,									-- if strength is less than this, the unit will not contribute to training and cant be damaged by training
	...
	BASE_SPOTTING_FROM_NAVY = 6,									-- base spotting percentage that comes from task forces in area
	...
	BASE_SPOTTING_FROM_AIR = 7,									-- base spotting percentage that comes from air-wings in area
	...
	SPOTTING_MULTIPLIER_FOR_SUB = 0.5,								-- task force sub spotting value is multiplied by this and added to spotting percentage every hour
	...
	MAX_ESCAPE_SPEED_FROM_COMBAT_DURATION = 0.2,					-- max escape speed that will be gained from combat duration
	ESCAPE_SPEED_SUB_BASE = 0.25,									-- subs get faster escape speed. gets replaced by hidden version below if hidden
	ESCAPE_SPEED_HIDDEN_SUB = 0.7,									-- hidden subs get faster escape speed

	SUB_DETECTION_CHANCE_BASE = 4,									-- to start spotting a submarine, a dice is rolled and checked if it succeeds this percentage. if not, that enemy sub force won't be spotted on this tick
	SUB_DETECTION_CHANCE_BASE_SPOTTING_EFFECT = 0.6,				-- effect of base spotting for initial spotting of pure submarine forces. this along with next value is added together and rolled against a random to start spotting
	SUB_DETECTION_CHANCE_SPOTTING_SPEED_EFFECT = 2.5,				-- effect of spotting speed for initial spotting of pure submarine forces. this along with prev value is added together and rolled against a random to start spotting
	...
	BASE_NAVAL_INVASION_SPOTTING_SPEED = 0.15,						-- daily base spotting speed against unit transfers
	...
	SPOTTING_SPEED_EFFECT_FOR_INITIAL_UNIT_TRANSFER_SPOTTING = 4.0, -- same as SPOTTING_SPEED_EFFECT_FOR_INITIAL_CONVOY_SPOTTING, but for naval transfer convoys
	BASE_SPOTTING_EFFECT_FOR_INITIAL_NAVAL_INVASION_SPOTTING = 6.0,    -- same as BASE_SPOTTING_EFFECT_FOR_INITIAL_CONVOY_SPOTTING, but for naval invasion convoys
	SPOTTING_SPEED_EFFECT_FOR_INITIAL_NAVAL_INVASION_SPOTTING = 12.0,  -- same as SPOTTING_SPEED_EFFECT_FOR_INITIAL_CONVOY_SPOTTING, but for naval invasion convoys
	...
	POSITIONING_PENALTY_FOR_SHIPS_JOINED_COMBAT_AFTER_IT_STARTS		= 0.03, -- each ship that joins the combat will have this penalty to be added into positioning
	...
	SUBMARINE_REVEAL_ON_MINIMUM_POSITIONING                         = 0.1,  -- submarine reveal change on 0% positioning 
	...
	ANTI_AIR_MULT_ON_INCOMING_AIR_DAMAGE							= 0.25,
	...
	SCREEN_RATIO_FOR_FULL_SCREENING_FOR_CONVOYS 					= 0.2,	-- this screen ratio to num convoys is needed for full screening beyond screen line
	...
	SURFACE_DETECTION_STAT_FOR_SHIP_TO_BE_PATROL = 25,					-- amount of surface detection required for a ship to be considered as part of a patrol task force
	DEPTH_CHARGE_STAT_FOR_SHIP_TO_BE_SUB_HUNTER = 20,					-- amount of depth charge required for a ship to be considred a sub hunter and so good for convoy escort
	...
	HEAVY_GUN_ATTACK_TO_SHORE_BOMBARDMENT							= 0.15,  -- heavy gun attack value is divided by this value * 100 and added to shore bombardment modifier 
	...
	DEPTH_CHARGES_HIT_PROFILE 										= 60.0,	    -- hit profile for depth charges
	
	
	CONVOY_HIT_PROFILE												= 105.0,  	-- convoys has this contant hit profile
	HIT_PROFILE_MULT 												= 100.0,  	-- multiplies hit profile of every ship
	HIT_PROFILE_SPEED_FACTOR										= 1,		-- factors speed value when determining it profile (Vis * HIT_PROFILE_MULT * Ship Hit Profile Mult)
	...
	CONVOY_DEFENSE_MAX_CONVOY_TO_SHIP_RATIO							= 7.0,		-- each ship in convoy defense mission can at most cover this many convoys without losing efficiency
	...
	NAVAL_COMBAT_SUB_DETECTION_FACTOR                               = 1.05,      -- balance value for sub detection in combat by ships
	...
	MAX_NUM_HOURS_TO_WAIT_AT_ALLY_DOCKYARDS_FOR_REPAIRS 			= 24,		-- taskforces will wait at most this amount of hours in ally bases for repairs before switching to another base for repairs
	...
	NAVAL_COMBAT_AIR_CAPITAL_TARGET_SCORE = 25,
	NAVAL_COMBAT_AIR_CARRIER_TARGET_SCORE = 100,
...
NRailwayGun = {
	...
	OUT_OF_SUPPLY_SPEED = -0.6,						-- Max speed reduction from supply for railway guns
...
NAI = {
	...
	DIPLOMACY_SEND_MAX_FACTION = 0.08,			-- Country should not send away more units than this as expeditionaries
	...
	RESEARCH_LAND_DOCTRINE_NEED_GAIN_FACTOR = 0.13, -- Multiplies value based on relative military industry size / country size.
	...
	RESEARCH_AHEAD_BONUS_FACTOR = 2.7,          -- To which extent AI should care about ahead of time bonuses to research
	RESEARCH_BONUS_FACTOR = 5, 				-- To which extent AI should care about bonuses to research
	...
	MAX_AHEAD_RESEARCH_PENALTY = 3.3,
	RESEARCH_AHEAD_OF_TIME_FACTOR = 6.5, 		-- To which extent AI should care about ahead of time penalties to research
	...
	PRODUCTION_EQUIPMENT_SURPLUS_FACTOR_GARRISON = 0.1,	-- Base value for how much of currently used equipment the AI will at least strive to have in stock for garrison forces
	...
	DEFAULT_MODULE_VARIANT_CREATION_XP_CUTOFF_NAVY = 150,	-- Same as above but for the ship designer.
	DEFAULT_MODULE_VARIANT_CREATION_XP_CUTOFF_AIR = 25,		-- Same as above but for the plane designer.
	DEFAULT_LEGACY_VARIANT_CREATION_XP_CUTOFF_LAND = 180,	-- Army XP needed before attempting to create a variant of a type that uses the legacy upgrades system. ai_strategy supports land_xp_spend_priority upgrade_xp_cutoff. If none is set, this define is used instead.
	DEFAULT_LEGACY_VARIANT_CREATION_XP_CUTOFF_NAVY = 150,	-- Same as above but for navy XP and navy_xp_spend_priority.
	DEFAULT_LEGACY_VARIANT_CREATION_XP_CUTOFF_AIR  = 150,	-- Same as above but for air XP and air_xp_spend_priority.
	...
	DESIRE_USE_XP_TO_UNLOCK_NAVAL_DOCTRINE = 0.4,   -- How quickly is desire to unlock naval doctrines accumulated?
	DESIRE_USE_XP_TO_UNLOCK_AIR_DOCTRINE = 0.5,     -- How quickly is desire to unlock air doctrines accumulated?
	DESIRE_USE_XP_TO_UPDATE_LAND_TEMPLATE = 0.0,    -- How quickly is desire to update/create templates accumulated?
	...
	DAYS_BETWEEN_CHECK_BEST_DOCTRINE = 12;       -- Recalculate desired best doctrine to unlock with this many days inbetween.
	DAYS_BETWEEN_CHECK_BEST_TEMPLATE = 12;       -- Recalculate desired best template to upgrade with this many days inbetween.
	DAYS_BETWEEN_CHECK_BEST_EQUIPMENT = 12;      -- Recalculate desired best equipment to upgrade with this many days inbetween.
	...
	PLAN_VALUE_TO_EXECUTE = 0.15,				-- AI will typically avoid carrying out a plan it below this value (0.0 is considered balanced).
	...
	CALL_ALLY_FASCIST_DESIRE = -15,				-- Desire to call ally added for fascist AI
	...
	JOIN_ALLY_COMMUNIST_DESIRE = 50,			-- Desire to join ally added for communist AI
	...
	NEIGHBOUR_WAR_THREAT_FACTOR = 1.0, 		-- How much increase in threat does AI feel against neighbours who are at war
	...
	MAX_ALLOWED_NAVAL_DANGER = 500,				-- #Was 80.... AI will ignore naval paths that has danger value of above this threshold while assigning units
	...
	MIN_FUEL_RATIO_TO_NOT_IGNORE_STRIKE_FORCE_COST = 0.01, -- ai will still assign strike forces unless fuel ratio drops below this one
	MIN_FUEL_RATIO_TO_NOT_IGNORE_INVASION_SUPPORT_COST = 0.01, -- ai will still naval invasion support forces unless fuel ratio drops below this one
	...
	SURRENDER_LEVEL_TO_PULL_EXPEDITIONARIES_BACK = 0.15,			-- AI will pull expeditioniries back if its surrender level is above this ratio
	SURRENDER_LEVEL_TO_NOT_SEND_EXPEDITIONARIES = 0.05,			-- AI will not send expeditioniries if its surrender level is above this ratio
	...
	INVASION_COASTAL_PROVS_PER_ORDER = 22,				-- AI will consider one extra invasion per number of provinces stated here (num orders = total coast / this)
	...
	REGION_THREAT_LEVEL_TO_AVOID_REGION = 400,		-- How much threat must be generated in region ( by REGION_THREAT_PER_SUNK_CONVOY ) so the AI will decide to mark the region as avoid
	...
	REGION_CONVOY_DANGER_DAILY_DECAY = 8,				-- When convoys are sunk it generates threat in the region which the AI uses to prio nalval missions
	...
	PLAN_ACTIVATION_PLAYER_WEIGHT_FACTOR = 1,		-- AI countries will hold on activating plans if player controlled countries have plans in the same location.
	...
	TRANSPORTS_PER_PARATROOPER = 100,					-- Currently unused
	...
	PRODUCTION_MAX_PROGRESS_TO_SWITCH_NAVAL = 0.09,		-- AI will not replace ships being built by newer types if progress is above this
	PRODUCTION_WAIT_TO_FINISH_IF_EXPENSIVE = 0.09,      -- If produced item is expensive (producing less than one/week), wait to finish item if progress is above this
	...
	MIN_MANPOWER_TO_BUILD_UNITS = 0.98,					-- Cancel unit production if below this to get resources out to units in the field
	...
	INVASION_DISTANCE_RANDOMNESS = 60,					-- This higher the value, the more unpredictable the invasions. Compares to actual map distance in pixels.
	...
	LAND_COMBAT_ANTI_LOGISTICS_PER_ENEMY_ARMY = 0.05,    -- Amount of CAS planes requested per enemy army for anti-logistics
	...
	XP_RATIO_REQUIRED_TO_RESEARCH_WITH_XP = 1.2,		-- AI will at least need this amount of xp compared to cost of a tech to reserch it with XP
	RESEARCH_WITH_XP_AI_WEIGHT_MULT = 1.3, 				-- AI will bump score of a research with this mult if it can use XP
	...
	STR_BOMB_MIN_ENEMY_FIGHTERS_IN_AREA = 600,			-- If amount of enemy fighters is higher than this mission won't perform
	...
	UPGRADES_DEFICIT_LIMIT_DAYS = 800,                           -- Ai will avoid upgrading units in the field to new templates if it takes longer than this to fullfill their equipment need
	...
	MAX_SCREEN_TASKFORCES_FOR_CONVOY_DEFENSE_MAX_CONVOY_THREAT = 2000,-- AI will increase screen assignment for escort missions as threate increases
	...
	MAX_FACTORY_TO_SPARE_FOR_MISSION_FUEL_TRADE_IN_PEACE = 0.05, 			-- amount of factories to spend on oil trade in case of fuel need for missions in peace time
	MAX_FACTORY_TO_SPARE_FOR_CRITICAL_MISSION_FUEL_TRADE_IN_PEACE = 0.15, 	-- amount of factories to spend on oil trade in case of fuel need for prio missions in peace time
	MAX_FACTORY_TO_TRADE_FOR_FUEL_IN_PEACE = 0.25,
	...
	MAX_FUEL_CONSUMPTION_RATIO_FOR_AIR_TRAINING = 0.2,			-- ai will use at most this ratio of affordable fuel for air training
	...
	NAVAL_INVADED_AREA_PRIO_MULT = 1.25,								-- fronts that belongs to recent invasions gets more prio
	...
	FAILED_INVASION_AVOID_DURATION = 75,                   -- after a failed invasion, AI will down-prioritize invading the same area again for this number of days
	...
	MAX_INVASION_SIZE = 6,									-- max invasion group size
	...
	AI_UPDATE_ROLES_FREQUENCY_HOURS = 96;               -- Update the roles for a country AI this often (affects performance)

	UPDATE_SUPPLY_BOTTLENECKS_FREQUENCY_HOURS = 168;     -- Check for and try to fix supply bottlenecks this often. (168 hours = 1 week)
	FIX_SUPPLY_BOTTLENECK_SATURATION_THRESHOLD = 0.8;  -- Try to fix supply bottlenecks if supply node saturation exceeds this value.
	...
	ARMY_LEADER_ASSIGN_KEEP_LEADER = 1900,                       -- Score for keeping the leader if already assigned
	...
	ARMY_LEADER_ASSIGN_EXILED_LEADS_OWN_EXILED_TROOPS = 200,    -- If exiled leader, increase chance of leading army with exiled troops from same country as the leader
...
NOperatives = {
	...
	PROPAGANDA_DEFENSE_FACTOR = 0.1,					-- Multiplied to the target's defense before being subtracted from the Stability/WarSupport drift caused by an operative
	...
	PROPAGANDA_DAILY_XP_GAIN = 0.250,
	...
	ROOT_OUT_RESISTANCE_DAILY_XP_GAIN = 0.168,
	...
	OPERATIVE_BASE_DIPLOMATIC_PRESSURE_AI_ACCEPTANCE_SCORE_DRIFT = 0.5,	-- The daily change in the amount of opinion requiered to join a faction
	OPERATIVE_BASE_DIPLOMATIC_PRESSURE_TENSION_REQUIREMENTS_DRIFT = 0.0015,	-- The daily change in world tension requiered to join a faction
	DIPLOMATIC_PRESSURE_MAX_AI_ACCEPTANCE_SCORE_INCREASE = 25.0,	-- the maximum amount of ai acceptance score from diplomatic pressure
...
NIntel = {
	...
	NAVY_INTEL_MINE_DAMAGE_REDUCTION_FACTOR_MAX_INTEL_FOR_BONUS = 80, -- t this intel NAVAL_MINES_INTEL_DIFF_FACTOR will be applied fully
...
NSupply = {
	...
	CAPITAL_SUPPLY_BASE = 2.0, -- base supply for capital
	...
	SUPPLY_HUB_FULL_MOTORIZATION_BONUS = 2,
	-- How many trucks does it cost to fully motorize a hub
	SUPPLY_HUB_FULL_MOTORIZATION_TRUCK_COST = 200.0,
	...
	NAVAL_BASE_FLOW = 6, -- max output/input of a naval node is limited by this base value + additional ratio for each level
	NAVAL_FLOW_PER_LEVEL = 3, -- max output/input of a naval node is limited by previous base value + this define per its level
...
NAITheatre = {
	...
	AI_THEATRE_SUPPLY_CRISIS_LIMIT = 0.2,                               -- If a unit is standing in an area with this supply ratio it will try to escape
