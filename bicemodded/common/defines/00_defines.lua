...
NAir = {
	...
	COMBAT_SITUATION_WIN_CHANCE_FROM_STATS = 0.35, 		-- How much good stats difference (speed+agility) affects the winning chance (situation win = give a hit, not necessary kill).( Naval air combat only )
	COMBAT_SITUATION_WIN_CHANCE_FROM_GANG = 0.35, 		-- How much bonus gives the airplanes amount advantage (2vs1), to winning the situation.( Naval air combat only )
	...
	NAVAL_STRIKE_TARGETTING_TO_AMOUNT = 0.47,			-- Balancing value to convert the naval_strike_targetting equipment stats to chances of how many airplanes managed to do successfull strike.
	NAVAL_STRIKE_DAMAGE_TO_STR = 8.0,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Strength reduction.
	NAVAL_STRIKE_DAMAGE_TO_ORG = 4.0,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Organisation reduction.
	...
	AIR_COMBAT_FINAL_DAMAGE_SCALE = 0.35,               -- % how many max disrupted only planes are alloed to die in a single combat
	...
	ANTI_AIR_PLANE_DAMAGE_FACTOR = 0.24,					-- Anti Air Gun Damage factor
	...
	NAVAL_COMBAT_EXTERNAL_PLANES_JOIN_RATIO_PER_DAY = 0.45, -- max extra plane % that can join every day
...
NNavy = {
	...
	EXPERIENCE_FACTOR_CARRIER_GAIN = 0.005,							-- Xp gain by carrier ships in the combat
	...
	NAVAL_SUPREMACY_INTEL_LOW = 0,								-- we need more intel than this to get any supremacy
...
NAI = {
	GARRISON_FRACTION = 0.26, 					-- up to 10 was0.26How large part of a front should always be holding the line rather than advancing at the enemy
	...
	NAVAL_MAX_PRIO_THEATRES = 7,				-- A nation may have a large number of theatres, but all of them having stationed/assigned navy is redundant
	NAVAL_THEATRE_PRIO_CAPITAL_SCORE = 6,		-- Weight of capital when calculating naval theatre assignment
	NAVAL_THEATRE_PRIO_NAVAL_BASE_SCORE = 2,	-- Weight of naval bases when calculating naval theatre assignment
	...
	MAX_ALLOWED_NAVAL_DANGER = 10,				-- #Was 80.... AI will ignore naval paths that has danger value of above this threshold while assigning units
	TRANSFER_DANGER_HOSTILE_SHIPS = 80, 		-- max danger from complete enemy naval supriority over ai in an area
	...
	CONVOY_NEED_SAFETY_BUFFER = 4,					-- AI will try and keep 15% more convoys than what it needs.
	...
	MISSING_CONVOYS_BOOST_FACTOR = 22.0,					-- The more convoys a country is missing, the more resources it diverts to cover this.
	...
	ATTACK_HEAVILY_DEFENDED_LIMIT = 0.8,				-- AI will not launch attacks against heavily defended fronts unless they consider to have this level of advantage (1.0 = 100%)
