...
NDiplomacy = {
	...
	TENSION_CIVIL_WAR_IMPACT = 0.16,										-- civil war multiplier on tension.
	...
	TENSION_TIME_SCALE_MIN = 0.45,										-- Timed tension scale won't decrease under this value
	...
	JOINING_NAP_WAR_PENALTY = 0.15,										-- National Unity penalty for breaking non-breakable NAP
	BREAKING_GUARANTEE_PENALTY = 0.15,									-- National Unity penalty for breaking guarantee
	...
	NOT_READY_FOR_WAR_BASE = -450,										-- AI should be unwilling to enter accept a call to war if not ready for war against the relevant enemies.
...
NProduction = {
	...
	ANNEX_FIELD_EQUIPMENT_RATIO = 0.45,						-- How much equipment from deployed divisions will be transferred on annexation
...
NMilitary = {
	...
	ATTRITION_EQUIPMENT_LOSS_CHANCE = 0.09,		   			-- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	...
	BASE_CHANCE_TO_AVOID_HIT = 88,                			-- Base chance to avoid hit if defences left.
	...
	BASE_FORT_PENALTY = -0.40, 					   			-- fort penalty
	...
	DIG_IN_FACTOR = 0.015,						   			-- bonus factor for each dug-in level
	LEADER_SKILL_FACTOR = 0.04,                    			-- bonus per skill level of leader
	...
	PLAN_CONSIDERED_BAD = 0.3,					-- Plan evaluations below this value are considered unsafe
...
NAir = {
	...
	CAPACITY_PENALTY = 0.8,								-- scales penalty of having overcrowded bases.
	...
	DISRUPTION_FACTOR = 2.0,							-- multiplier on disruption damage to scale its effects on planes
...
NNavy = {
	...
	COMBAT_BASE_HIT_CHANCE = 0.09,										-- base chance for hit
	...
	COMBAT_ARMOR_PIERCING_DAMAGE_REDUCTION = -0.75,						-- BICE 0.65,DNM 0.9, to be consistent with changes to vanilla, makes armour more important, as it shold be -- All damage reduction % when target armor is >= then shooter armor piercing.
	REPAIR_AND_RETURN_PRIO_LOW = 0.7,								-- #DNM, vanilla 0.2 -- % of total Strength. When below, navy will go to home base to repair.
	REPAIR_AND_RETURN_PRIO_MEDIUM = 0.8,							-- #DNM, vanilla 0.5 -- % of total Strength. When below, navy will go to home base to repair.
	REPAIR_AND_RETURN_PRIO_HIGH = 0.9,								-- % of total Strength. When below, navy will go to home base to repair.
	REPAIR_AND_RETURN_PRIO_LOW_COMBAT = 0.6,						-- #DNM, vanilla 0.1, far too low -- % of total Strength. When below, navy will go to home base to repair (in combat).
	REPAIR_AND_RETURN_PRIO_MEDIUM_COMBAT = 0.65,					-- #DNM, vanilla 0.3, felt too low -- % of total Strength. When below, navy will go to home base to repair (in combat).
	REPAIR_AND_RETURN_PRIO_HIGH_COMBAT = 0.8,						-- #DNM, vanilla 0.5 -- % of total Strength. When below, navy will go to home base to repair (in combat).
	REPAIR_AND_RETURN_AMOUNT_SHIPS_LOW = 0.75,						-- #DNM, vanilla 0.2 -- % of total damaged ships, that will be sent for repair-and-return in one call.
	REPAIR_AND_RETURN_AMOUNT_SHIPS_MEDIUM = 0.85,					-- #DNM, vanilla 0.4 -- % of total damaged ships, that will be sent for repair-and-return in one call.
	REPAIR_AND_RETURN_AMOUNT_SHIPS_HIGH = 1.0,						-- #DNM, vanilla 0.8 -- % of total damaged ships, that will be sent for repair-and-return in one call.
	...
	NAVY_VISIBILITY_BONUS_ON_RETURN_FOR_REPAIR = 0.5,					-- DNM - original value 0.9 -- Multiplier for the surface/sub visiblity when the heavily damaged fleet is returning to the home base for reparation. 1.0 = no bonus. 0.0 = invisible.
...
NAI = {
	...
	DIPLOMACY_FACTION_WRONG_IDEOLOGY_PENALTY = 25, 		-- AI penalty for diplomatic faction actions between nations of different ideologies
	...
	DIVISION_UPGRADE_MIN_XP = 25.0,						-- Minimum XP before attempting to upgrade a division template.
	...
	PLAN_ATTACK_MIN_STRENGTH_FACTOR_HIGH = 0.35,
	...
	RELATIVE_STRENGTH_TO_INVADE_DEFENSIVE = 0.5,		-- Compares the estimated strength of the country/faction compared to it's enemies to see if it should invade or stay at home to defend, but while being a defensive country.
	...
	ENTRENCHMENT_WEIGHT = 2.0,				    		-- DNM, original 100.0 (ai overhaul mod) -- AI should favour units with less entrenchment when assigning units around.
	...
	PLAN_FACTION_WEAK_TO_ABORT = 0.5,					-- % or more of units in an order to consider ececuting the plan
	...
	UNIT_ASSIGNMENT_TERRAIN_IMPORTANCE = 5.0,			-- DNM, original 10 -- Terrain score for units are multiplied by this when the AI is deciding which front they should be assigned to
	
	MIN_STATE_VALUE_TO_PROTECT = 9.0,					-- When AI is considering which states to protect it looks at state values to consider if it is worth it.
	...
	NAVAL_MISSION_MIN_FLEET_SIZE = 15,						-- AI will not send too small fleets on missions. Ignored if total number of ships country has is below	this.
	NAVY_PREFERED_MAX_SIZE = 40,							-- DNM, original 20 - BICE thought 50 was good as well.  AI will generally attempt to merge fleets into this size, but as a soft limit.
	...
	CONVOY_NEED_SAFETY_BUFFER = 0,						-- AI will try and keep 15% more convoys than what it needs.
	...
	MAX_DISTANCE_NALAV_INVASION = 300.0,					-- DNM, changed from 500 (AI Mod) -- AI is extremely unwilling to plan naval invasions above this naval distance limit.
	...
	MAX_SUPPLY_DIVISOR = 0.5,								-- DNM, original 2 (AI mod) To make sure the AI does not overdeploy divisions. Higher number means more supply per unit.
	...
	STATE_CONTROL_FOR_AREA_DEFENSE = 0.45,					-- To avoid AI sending area defense to area with very little foothold
	...
	AIR_SUPERIORITY_MISSION_FACTOR = 1.5,					-- AI air superiority mission factor
	...
	KAMIKAZE_MISSION_FACTOR = 1.5,							-- AI naval kamikaze mission factor
	PORT_STRIKE_MISSION_FACTOR = 1.5,						-- AI port strike mission factor
	INDUSTRY_IC_AIR_IMPORTANCE_FACTOR = 0.02,				-- AI very specific IC cost factor for air missions
	
	AIR_SCORE_DISTANCE_IMPACT = 0.15,						-- Effect of distance applied to the score calculations
	DAYS_BETWEEN_AIR_PRIORITIES_UPDATE = 5,					-- Amount of days between air ai updates priorities for air wings ( from 1 to N )
